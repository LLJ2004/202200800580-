...
enum UserType SDU_Restaurant_Welcome(void)
{
    ...
    while (!(userType == admin || userType == waiter))
    {
        ...
        {
            printf("\n\n\n");
            printf(" -------- 欢 迎 使 用 山 威 餐 饮 服 务 员 系 统 ---------\n\n\n\n");
        ...
        {
            printf("\n\n\n");
            printf(" -------- 欢 迎 使 用 山 威 餐 饮 管 理 员 系 统 ---------\n\n\n\n");
            Loading_Menu(ID_input, password_input);
            if (Check_ID_Passwd(ID_input, password_input, &userType, Choice))
            {
                userType = admin;
...
void Admin_Waiter_Change(void)
{
    while (1)
    {
        ...
        printf(" ==========================================================\n\n");
        printf("  输 入 \"add [工 号] [姓 名] \" 添 加 员 工 \n\n");
        ...
        if (strcmp(cmd, "add") == 0)
        {
            ...
            strcpy_s(newWaiter.passwd, sizeof(newWaiter.passwd), "123456"); //密码初始化
            newWaiter.profit = 0;
...
void Admin_VIP_Change(void)
{
    while (1)
    {
        ...
        {
            if (!(0 == SDU_Restaurant.vipsCount)) //判断 VIP库是否为空
            {
                int cardnum;
                scanf_s(" %d", &cardnum);
...
void Waiter_Seat_Select(void)
{
    ...
    while (1)
    {
        ...
        strcpy_s(newcustomer.address, sizeof(newcustomer.address), EMPTY_ADDRESS); // 复制空地址信息到 newcustomer.address
